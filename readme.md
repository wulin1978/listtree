...
> v-listtree 一共有7个参数：listData、open、indent、spacing、icon、iconSize、animation，通过配置这7个参数可以使 v-listtree 适应不同类型的菜单需求，为了描述方便，这里假设你要插入本插件的组件为mytree.vue(建议用空组件来引入本插件，然后在需要使用本本插件的地方直接import该组件)
